...
export const en = {
  ...
  jobs: {
    ...
    description: (hours: number) =>
      `Monitor a running export, then download it. Links expire ${hours}h after completion \u2014 Sync mints a fresh one.`,
    monitor: "Monitor",
    polling: "Polling every 10s, easing to 30s until the job finishes",
    pasteId: "Paste a job ID, or pick one from the tables below",
    ...
    expires: (when: string) => `Expires ${when} \u00b7 no auth needed, treat the link as sensitive`,
    ...
    nothingWatched: "Nothing being watched.",
    buildAQuery: "Build a query",
    andStartExport: " and start an export, or track a job ID above.",
    syncedRefreshed: (id: string, until: string) =>
      `Synced ${id} \u2014 download link refreshed (valid until ${until}).`,
    syncedStatus: (id: string, status: string) => `Synced ${id} \u2014 status is ${status}.`,
    ...
    sessionNote: "Jobs started or tracked from this browser, kept in localStorage",
    historyNote: (user: string, total: number) => `User ${user} \u00b7 ${total} job(s) returned`,
    noneTracked: "No jobs tracked in this browser yet.",
